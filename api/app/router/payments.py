...
from app.schema.payment import PaymentCreate, PaymentOut, PaymentUpdate
...
def list_payments(company_id: uuid.UUID | None = Query(default=None), db: Session = Depends(get_db)):
    return payment_service.list_payments(db, company_id)
