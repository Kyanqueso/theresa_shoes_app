    ...
    <div className="rounded-2xl bg-accent p-6">
      <div className="flex items-start justify-between gap-4">
        <div className="flex-1 text-center">
          ...
          {description && <p className="mx-auto mt-2 max-w-2xl text-sm text-gray-600">{description}</p>}
          {instructionText && <p className="mt-3 text-xs text-gray-500">{instructionText}</p>}
        </div>
        {onManageClick && (
          <button
            type="button"
      ...
      <div className="mt-5">
        ...
        {available.length === 0 ? (
          <p className="mt-2 text-sm text-gray-400">None available yet.</p>
        ) : (
          <div className="mt-2 grid grid-cols-3 gap-3 rounded-lg bg-white p-3 sm:grid-cols-4 md:grid-cols-5">
            {available.map((item) => (
              <SwatchTile
                key={item.id}
                item={item}
      ...
      {unavailable.length > 0 && (
        <div className="mt-4">
          ...
          <div className="mt-2 grid grid-cols-3 gap-3 rounded-lg bg-gray-100 p-3 sm:grid-cols-4 md:grid-cols-5">
